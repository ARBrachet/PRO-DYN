...
class MIC(nn.Module):
    ...
    def __init__(self, feature_size=512, n_heads=8, dropout=0.05, decomp_kernel=[17, 49], conv_kernel=[12, 16],
                 isometric_kernel=[17, 49], device=device):
...
class MICN_DYN(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # seasonal component through the regression linear layer, then feeds the seasonal block
        seasonal_regr = self.regression(seasonal_init_enc.permute(0, 2, 1)).permute(0, 2, 1)
        seasonal_init_dec = torch.cat([seasonal_init_enc[:, -self.seq_len:, :], seasonal_regr], dim=1)
